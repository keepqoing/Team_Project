...
public class DFS {
	...
	public void dfs(Class_Info[] Class_List, int current, String currentSum, String currentTime, ArrayList<String> Class_Num_List, ArrayList<String> Class_Time_List, int creditlow, int creditupp, int numOfday, boolean[] chk) {
		if(current == 6) return;
		...
		//이번 원소를 포함시키지 않고 시도========================================
		dfs(Class_List,current+1,currentSum,currentTime,Class_Num_List,Class_Time_List,creditlow,creditupp,numOfday,chk);
		...
		if(creditSum>=creditlow && creditSum<creditupp && cnt == numOfday) {
			Class_Num_List.add(currentSum);
			Class_Time_List.add(currentTime);
		}
